...
/**
 * Publisher -> Detroit Mag
 */
class Publisher_Theme_Style_Detroit_Mag extends Publisher_Theme_Style {
	...
	function panel_std( $fields ) {
		...
		include PUBLISHER_THEME_PATH . 'includes/styles/detroit-mag/panel-std.php';
	...
	function register_assets() {
		...
		bf_enqueue_style(
			'publisher-theme-detroit-mag',
			bf_append_suffix( Publisher_Theme_Styles_Manager::get_uri( 'detroit-mag/style' ), '.css' ),
			array( 'publisher' ),
			bf_append_suffix( Publisher_Theme_Styles_Manager::get_path( 'detroit-mag/style' ), '.css' ),
			Better_Framework()->theme()->get( 'Version' ) . mt_rand(100,10000)
		);
	}
	...
	public function register_tinymce_assets() {
		...
		bf_enqueue_tinymce_style( 'registered', 'publisher-theme-detroit-mag' );
	}
...
} // Publisher_Theme_Style_Detroit_Mag
