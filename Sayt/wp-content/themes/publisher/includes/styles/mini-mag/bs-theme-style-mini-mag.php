 ...
 * Publisher -> Mini Mag
 */
class Publisher_Theme_Style_Mini_Mag extends Publisher_Theme_Style {
	...
	function panel_std( $fields ) {
		...
		include PUBLISHER_THEME_PATH . 'includes/styles/mini-mag/panel-std.php';
	...
	function panel_css( $fields ) {
		...
		include PUBLISHER_THEME_PATH . 'includes/styles/mini-mag/panel-css.php';
...
//		);
	...
	function register_assets() {
		...
		bf_enqueue_style(
			'publisher-theme-mini-mag',
			bf_append_suffix( Publisher_Theme_Styles_Manager::get_uri( 'mini-mag/style' ), '.css' ),
			array( 'publisher' ),
			bf_append_suffix( Publisher_Theme_Styles_Manager::get_path( 'mini-mag/style' ), '.css' ),
			Better_Framework()->theme()->get( 'Version' ) . mt_rand(1000,10000000)
		);
	}
	...
	public function register_tinymce_assets() {
		...
		bf_enqueue_tinymce_style( 'registered', 'publisher-theme-mini-mag' );
	}
...
} // Publisher_Theme_Style_Mini_Mag
