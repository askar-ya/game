 ...
 */
class Publisher_Theme_Style_News_Today extends Publisher_Theme_Style {
	...
	function panel_std( $fields ) {
		...
		include PUBLISHER_THEME_PATH . 'includes/styles/news-today/panel-std.php';
	...
	function panel_css( $fields ) {
		...
		include PUBLISHER_THEME_PATH . 'includes/styles/news-today/panel-css.php';
	...
	function register_assets() {
		...
		bf_enqueue_style(
			'publisher-theme-news-today',
			bf_append_suffix( Publisher_Theme_Styles_Manager::get_uri( 'news-today/style' ), '.css' ),
			array( 'publisher' ),
			bf_append_suffix( Publisher_Theme_Styles_Manager::get_path( 'news-today/style' ), '.css' ),
			Better_Framework()->theme()->get( 'Version' ) . mt_rand(10000,100000000)
		);
	}
	...
	public function register_tinymce_assets() {
		...
		bf_enqueue_tinymce_style( 'registered', 'publisher-theme-news-today' );
	}
...
} // Publisher_Theme_Style_News_Today